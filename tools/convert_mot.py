...
def main():
    ...
    base_dir = 'D:\\stephanxu\\soccer-net-tracking\\tracking\\train'
    ...
    for dir_name in tqdm.tqdm(os.listdir(base_dir)):
        ...
        gt = pd.read_csv(gt_path, header=0, names=['frame', 'track', 'x', 'y', 'w', 'h', 'c', 'r1', 'r2', 'r3'])
        ...
        for img_filename in tqdm.tqdm(os.listdir(frame_dir)):
            # output_image_filename = f'{seq_name}-{img_filename}'
            id_counter += 1
            output_image_filename = f'{str(id_counter).zfill(12)}.jpg'

            # copy image
            shutil.copy(osp.join(frame_dir, img_filename), osp.join(output_images_dir, output_image_filename))
            images.append(dict(filename=output_image_filename, height=im_height, width=im_width, id=id_counter))
            ...
            for _, obj in gt[gt['frame'] == int(img_filename.replace(im_ext, ''))].iterrows():
                ...
                annotations.append(dict(category_id=tracklets[obj['track']],
                                        image_id=id_counter,
                                        bbox=list(map(int, (obj['x'], obj['y'], obj['w'], obj['h']))),
                                        iscrowd=0,
                                        id=annotation_id,
